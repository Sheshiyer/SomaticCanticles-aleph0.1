...
async function awardAchievement(
  ...
  await db
    .update(achievements)
    .set({ unlockedAt: new Date().toISOString() }) // unlockedAt is text in schema
...
async function checkChapterAchievements(
  ...
  if (!await isAchievementUnlocked(userId, 'first_chapter', db)) {
    ...
    const countValue = Number(completedChapters[0]?.count || 0);
    result.progress.first_chapter = { current: countValue, target: 1, percentage: Math.min(countValue * 100, 100) };
  ...
  if (!await isAchievementUnlocked(userId, 'chapter_master', db)) {
    ...
    const countValue = Number(completedChapters[0]?.count || 0);
    result.progress.chapter_master = { current: countValue, target: totalChapters, percentage: Math.round((countValue / totalChapters) * 100) };

    if (countValue >= totalChapters) {
...
async function checkTimeBasedAchievements(
  ...
  if (!await isAchievementUnlocked(userId, 'morning_person', db)) {
    ...
    const countValue = Number(morningCompletions[0]?.count || 0);
    result.progress.morning_person = { current: countValue, target: 1, percentage: Math.min(countValue * 100, 100) };
  ...
  if (!await isAchievementUnlocked(userId, 'night_owl', db)) {
    ...
    const countValue = Number(nightCompletions[0]?.count || 0);
    result.progress.night_owl = { current: countValue, target: 1, percentage: Math.min(countValue * 100, 100) };
...
async function checkCompletionistAchievement(
  ...
  const countValue = Number(unlockedCount[0]?.count || 0);
  const requiredCount = ACHIEVEMENT_DEFINITIONS.length - 1; // All except completionist

  result.progress.completionist = { current: countValue, target: requiredCount, percentage: Math.round((countValue / requiredCount) * 100) };

  if (countValue >= requiredCount) {
...
export async function getAchievementDetails(
  ...
  return userAchievements.map(achievement => {
    const achievementType = achievement.achievementType as AchievementType;
    const definition = ACHIEVEMENT_DEFINITIONS.find(a => a.type === achievementType)!;
    const progress = result.progress[achievementType] || { current: 0, target: 1, percentage: 0 };
