...
app.post('/track', zValidator('json', trackProgressSchema), async (c) => {
  ...
  try {
    ...
    if (data.action === 'complete') {
      ...
      updateData.timeSpentSeconds = (currentProgress.timeSpentSeconds || 0) + (data.timeSpentSeconds || 0);
    } else {
      if (data.progressPercentage !== undefined) {
        updateData.completionPercentage = data.progressPercentage;
      }
      if (data.timeSpentSeconds !== undefined) {
        updateData.timeSpentSeconds = (currentProgress.timeSpentSeconds || 0) + data.timeSpentSeconds;
      }
    }
