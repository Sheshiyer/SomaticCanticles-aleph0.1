...
export async function recordActivity(
  ...
  if (!canExtend) {
    ...
    newStreakCount = (streak.currentCount || 0) + 1;
    freezeUsed = true;
  ...
  const newLongestStreak = Math.max(streak.longestCount || 0, newStreakCount);
  ...
  // Check for milestone
  const milestones = STREAK_CONFIG.milestones as readonly number[];
  const milestoneReached = milestones.includes(newStreakCount)
    ? newStreakCount
    : null;
...
export async function getStreakSummary(userId: string, db: DB): Promise<{
  ...
  const totalActiveStreaks = userStreaks.filter(s => (s.currentCount || 0) > 0).length;
...
export async function getStreaksAtRisk(userId: string, db: DB): Promise<StreakData[]> {
  ...
  return userStreaks.filter(streak => {
    ...
    if ((streak.currentCount || 0) === 0) return false;
