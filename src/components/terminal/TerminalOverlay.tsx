...
const TerminalOverlay: React.FC<TerminalOverlayProps> = ({ isOpen, onClose, onCommand, chapterId }) => {
    ...
    // Auto-trigger for Chapter 13: The Binary Heartbeat
    useEffect(() => {
        if (isOpen && chapterId === 13) {
            const timer = setTimeout(() => {
                setHistory(prev => [...prev, { type: "system", text: "INCOMING RAW DATA STREAM DETECTED..." }]);
                setTimeout(() => {
                    // We can't easily call processCommand here because it's defined inside the component and not exposed easily in this scope without moving it or using a ref.
                    // But we can just directly set history.
                    setHistory(prev => [...prev, { type: "system", text: "DECODING RAW STREAM..." }]);
                    setTimeout(() => {
                        setHistory(prev => [...prev, { type: "output", text: "01001001 00100000 01000001 01001101 00100000 01001100 01001001 01010011 01010100 01000101 01001110 01001001 01001110 01000111" }]);
                        setTimeout(() => {
                            setHistory(prev => [...prev, { type: "output", text: "TRANSLATION: I AM LISTENING" }]);
                        }, 1000);
                    }, 800);
                }, 1000);
            }, 1500);
    ...
    const processCommand = (cmd: string, args: string[]) => {
        switch (cmd) {
            ...
            case "color":
                {
                    const targetTheme = args[0] as "cyan" | "amber" | "green" | undefined;
                    if (targetTheme && ["cyan", "amber", "green"].includes(targetTheme)) {
